#!/usr/bin/env python3
"""
SetFit Entity Classification CLI - Memory Efficient Version

Optimized for large datasets that don't fit in GPU memory.
"""
...
class MemoryEfficientHierarchicalClassifier:
    """Memory-efficient hierarchical classifier for large datasets."""
    ...
    def subsample_for_memory(self, train_df, max_samples_per_class=100):
        """Subsample training data to fit in memory."""
        ...
        print(f"\nSubsampled from {len(train_df)} to {len(subsampled_df)} samples")
        new_class_counts = subsampled_df['hierarchical_label'].value_counts()
    ...
    def train(self, train_df, val_df, model_name="sentence-transformers/paraphrase-mpnet-base-v2",
              batch_size=8, num_epochs=3, max_samples_per_class=100, use_gpu=True):
        """Train SetFit model with memory-efficient settings."""
        ...
        # Subsample training data to fit in memory
        if len(train_df) > 800:  # Arbitrary threshold
            train_df = self.subsample_for_memory(train_df, max_samples_per_class)
        ...
        args = TrainingArguments(
            ...
            sampling_strategy="oversampling",  # Better for imbalanced data
        )
        ...
        # Train with timing and memory monitoring
        print(f"\nStarting training with batch_size={batch_size}, epochs={num_epochs}")
        if torch.cuda.is_available():
        ...
        try:
            ...
            print("GPU out of memory during training. Trying CPU fallback...")
            # Move model to CPU and retry
            self.model = self.model.to("cpu")
            trainer.model = self.model
            trainer.train()
            training_time = time.time() - start_time
            print(f"Training completed on CPU in {training_time:.2f} seconds ({training_time/60:.2f} minutes)")
    ...
    def predict(self, texts):
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                print("GPU OOM during prediction, using CPU...")
                self.model = self.model.to("cpu")
                batch_preds = self.model.predict(batch)
                predictions.extend(batch_preds)
...
def main():
    parser = argparse.ArgumentParser(description='Memory-efficient SetFit training')
    ...
    parser.add_argument('--batch_size', type=int, default=8,
                        help='Batch size (start small: 4, 6, 8)')
    parser.add_argument('--max_samples_per_class', type=int, default=100,
                        help='Maximum samples per class to avoid memory issues')
    ...
    # Train
    classifier = MemoryEfficientHierarchicalClassifier(min_examples_threshold=args.min_examples)
